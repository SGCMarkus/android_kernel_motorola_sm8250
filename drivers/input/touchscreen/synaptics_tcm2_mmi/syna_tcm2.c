...
int syna_set_fw_name(struct syna_tcm *tcm, char *name)
{
    ...
    if (len > FILE_NAME_LENGTH) {
        LOGD("vendor name is too long");
        return -EINVAL;
    }
    pos = strstr(name, ".img");
    if (pos == NULL)
        snprintf(tcm->fw_name, FILE_NAME_LENGTH, "%s", name);
    else {
        len = len - strlen(".img");
        snprintf(tcm->fw_name, len + 1, "%s", name);
    }

	return 0;
}

int syna_reflash_do_reflash(struct syna_tcm *tcm, char *name)
{
	int retval;
	...
	snprintf(fwname, FILE_NAME_LENGTH, "%s.img", tcm->fw_name);
	LOGD("Firmware name is %s\n", fwname);
 ...
 */
#ifdef STARTUP_REFLASH
struct void syna_dev_reflash_startup_work(struct work_struct *work)
{
	int retval;
	...
	tcm->update_flag = FIRMWARE_UPDATE_SUCCESS;
	delayed_work = container_of(work, struct delayed_work, work);
	tcm = container_of(delayed_work, struct syna_tcm, reflash_work);

	tcm_dev = tcm->tcm_dev;

	if (reflash_data->fw_name[0] != 0) {
		LOGD("Request firmware %s\n", reflash_data->fw_name);
		/* get firmware image */
		retval = request_firmware(&fw_entry,
				reflash_data->fw_name,
				tcm->pdev->dev.parent);
		if (retval < 0) {
			LOGE("Fail to request %s\n", reflash_data->fw_name);
			tcm->update_flag = FIRMWARE_UPDATE_FAILED;
			return;
		}
	}
