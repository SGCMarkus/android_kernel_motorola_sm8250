...
#ifndef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_FW_UPGRADE
...
#endif
static int cyttsp5_check_firmware_config_version(struct device *dev, u16 fw_config_ver_new)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	u16 fw_config_ver_img;

	fw_config_ver_img = ld->si->cydata.fw_ver_conf;

	dev_dbg(dev, "%s: img config vers:0x%04X new config vers:0x%04X\n",
		...
		struct cyttsp5_touch_firmware *fw)
{
	...
	fw_config_ver_new = get_unaligned_be16(fw->ver + 16);

	upgrade = cyttsp5_check_firmware_version(dev, fw_ver_new,
		fw_revctrl_new);
	
	if (upgrade <= 0)
		upgrade = cyttsp5_check_firmware_config_version(dev, fw_config_ver_new);
...
retry_bl:
	...
	if (upgrade) {
		rc = cyttsp5_upgrade_firmware(dev, fw->img, fw->size);
#ifdef PT_PANEL_ID_FROM_MFG
		/* An extra BL may be needed if default PID was wrong choice */
		if (!ld->si && retry--) {
			dev_warn(dev, "%s: An extra BL may be needed\n", __func__);
...
static int _pt_pip1_bl_from_file(struct device *dev)
{
	...
	rc = cmd->nonhid_cmd->read_us_file(dev, cd->pip2_us_file_path,
					    fw_img, &fw_size);
	if (rc) {
		dev_err(dev, "%s: No firmware provided to load\n",
			 __func__);
		goto exit;
	}
	...
	cyttsp5_upgrade_firmware(dev, &(fw_img[header_size + 1]),
...
{
	struct cyttsp5_loader_data *ld = container_of(bl_from_file,
			struct cyttsp5_loader_data, bl_from_file);
	struct device *dev = ld->dev;
		...
		struct device_attribute *attr, char *buf)
{
	...
	ld->builtin_bin_fw_status = false;
 ...
 *  be used to BL, for example: "echo /data/pt_fw > pip2_bl_from_file", and
 ...
 ******************************************************************************/
static ssize_t pt_pip_bl_from_file_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size <= 128) {
		...
		dev_info(dev, "%s:Path=%s, ptr_right=%s\n", __func__,
			 ptr_left, ptr_right);
