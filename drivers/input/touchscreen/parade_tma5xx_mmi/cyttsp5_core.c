...
#define MFG_WRITE_LENGTH 128
int cyttsp5_write_mfg_val_(struct cyttsp5_core_data *cd, u8 *data, u8 data_len)
{
	int rc;
	u16 actual_write_len;
	u16 mfg_data_size;
	u16 crc;
	u8 write_buf[MFG_WRITE_LENGTH];
	u8 cyttsp5_security_key[] = {0xA5, 0x01, 0x02, 0x03,
				     0xFF, 0xFE, 0xFD, 0x5A};

	if (data_len > MFG_WRITE_LENGTH)
		return -EINVAL;

	memset(write_buf, 0, MFG_WRITE_LENGTH);
	rc = cyttsp5_hid_output_suspend_scanning_(cd);
	if (rc) {
		dev_err(cd->dev, "%s: fail to suspend scan\n", __func__);
		goto exit;
	}

	rc = cyttsp5_hid_output_read_conf_block_(cd, 0, 4, CY_MDATA_EBID,
						 write_buf, &crc);
	if (rc) {
		dev_err(cd->dev, "%s: fail to suspend scan\n", __func__);
		goto exit;
	...
	memcpy(&write_buf[4], data, data_len);
	dev_info(cd->dev, "%s: Writing %x %x %x %x %x\n", __func__,
		write_buf[0], write_buf[1], write_buf[2], write_buf[3], write_buf[4]);

	mfg_data_size = get_unaligned_le16(&write_buf[0]);
	if (mfg_data_size > MFG_WRITE_LENGTH) {
		dev_err(cd->dev, "%s: MFG data size %d is not available\n", __func__, mfg_data_size);
		rc = -EINVAL;
		goto exit;
	}
	dev_info(cd->dev, "%s: MFG data size %d\n", __func__, mfg_data_size);
	crc = crc_ccitt_calculate(&write_buf[0], mfg_data_size);
	dev_info(cd->dev, "%s: MFG data size %d, 0x%04x\n", __func__, mfg_data_size, crc);
	write_buf[MFG_WRITE_LENGTH-2] = LOW_BYTE(crc);
	write_buf[MFG_WRITE_LENGTH-1] = HI_BYTE(crc);

	rc = cyttsp5_hid_output_write_conf_block_(
	    cd, 0, MFG_WRITE_LENGTH, CY_MDATA_EBID, write_buf,
	    &cyttsp5_security_key[0], &actual_write_len);
	if (rc) {
		dev_err(cd->dev, "%s: Fail to write MFG value r=%d\n", __func__,
			rc);
		goto exit;
	}

	if (actual_write_len != MFG_WRITE_LENGTH) {
		dev_err(cd->dev, "%s: Fail to write size=%d, actual_write=%d\n",
			__func__, MFG_WRITE_LENGTH, actual_write_len);
		rc = -EINVAL;
	}
...
exit:
	rc = cyttsp5_hid_output_resume_scanning_(cd);
	if (rc)
		dev_err(cd->dev, "%s: fail to resume scann\n", __func__);

	return rc;
}

static int _cyttsp5_mfg_value_(struct cyttsp5_core_data *cd, u8 *mfg_buffer, u8 mfg_num)
{
	int rc;
	u8* read_buf;
	u16 crc;

	read_buf = kzalloc(mfg_num + 4, GFP_KERNEL);

	rc = cyttsp5_hid_output_read_conf_block_(cd, 0, 4 + mfg_num, CY_MDATA_EBID,
						 read_buf, &crc);
	if (rc)
		goto exit;

	memcpy(mfg_buffer, &read_buf[4], mfg_num);
...
static __maybe_unused int cyttsp5_get_mfg_value_(struct cyttsp5_core_data *cd, u8 *mfg_buffer, u8 mfg_num)
{
	...
	rc = _cyttsp5_mfg_value_(cd, mfg_buffer, mfg_num);
	if (rc)
		goto exit;
...
error:
	dev_info(cd->dev, "%s: MFG_0:%02X\n", __func__, mfg_buffer[0]);
	return rc;
}
		...
		struct cyttsp5_hid_desc *desc)
{
	...
	cyttsp5_pr_buf(cd->dev, cd->response_buf, sizeof(struct cyttsp5_hid_desc), "HID Desc");
...
int cyttsp5_get_report_descriptor_(struct cyttsp5_core_data *cd)
{
	struct device *dev = cd->dev;
	u8 cmd[2];
	int rc;
 ...
 * touch report
 */
const struct cyttsp5_tch_abs_params tch_hdr_default[CY_TCH_NUM_HDR] = {
 ...
 * button report
 */
const struct cyttsp5_tch_abs_params tch_abs_default[CY_TCH_NUM_ABS] = {
	/* byte offset, size, min, max, bit offset, report */
 ...
 ******************************************************************************/
static void pt_init_pip_report_fields(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_sysinfo *si = &cd->sysinfo;
...
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
#ifdef PT_PANEL_ID_FROM_MFG
	int rc = 0;

/* Attempt to get sysinfo if not ready and panel_id is from XY pin */
	if (!cd->sysinfo.ready) {
		rc = cyttsp5_get_mfg_value_(cd, &cd->panel_id, 1);
		if (rc) {
			dev_err(dev,
				"%s: Error getting panel ID from MFG rc=%d\n",
				__func__, rc);
		}
...
#ifndef GET_PANEL_ID_FROM_BOOTLOADER
	/* Wait to skip bootloader mode */
	msleep(120);
#endif
...
#ifdef GET_PANEL_ID_FROM_BOOTLOADER
...
#ifdef PT_PANEL_ID_FROM_MFG
	rc = cyttsp5_get_mfg_value_(cd, &cd->mfg_data[0], MFG_DATA_NUM);
	if (rc)
		dev_err(cd->dev, "%s: failed to read MFG data rc=%d\n",
			__func__, rc);
#endif

	dev_info(cd->dev, "%s: Panel ID=0x%02x\n", __func__, cd->panel_id);
...
int _pt_read_us_file(struct device *dev, u8 *file_path, u8 *buf, int *size)
{
	...
	if (file_path == NULL || buf == NULL) {
		dev_err(dev, "%s: path || buf is NULL.\n", __func__);
		return -EINVAL;
	}
	...
	if (filp->f_op == NULL) {
		dev_err(dev, "%s: File Operation Method Error\n",
			__func__);
		rc = -EINVAL;
		goto exit;
	}
	...
	if (inode == NULL) {
		dev_err(dev, "%s: Get inode from filp failed\n",
			__func__);
		rc = -EINVAL;
		goto exit;
	}
	...
	if (file_len == 0) {
		dev_err(dev, "%s: file size error,file_len = %d\n",
			__func__, file_len);
		rc = -EINVAL;
		goto exit;
	}
	...
	if (read_len > PT_PIP2_MAX_FILE_SIZE) {
		dev_err(dev, "%s: file size ( %d ) exception.\n",
			__func__, read_len);
		rc = -EINVAL;
		goto exit;
	}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	memset(cd->pip2_us_file_path, 0, 128);
	memcpy(cd->pip2_us_file_path, "/data/ttdl/pt_fw.bin",
		sizeof("/data/ttdl/pt_fw.bin"));
